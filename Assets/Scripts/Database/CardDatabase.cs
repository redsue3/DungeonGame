...
public static class CardDatabase
{
    private static readonly Dictionary<string, Card> table = new Dictionary<string, Card>
    {
        ...
        // 기본(일반) 카드 — 직업 무관, 전 계층 공통 등장 (2026-07-05 추가)
        // 코스트는 2026-07-09에 전 직업 공용 매커닉으로 전환되며 실제 위력에 맞춰 재산정 (기존엔 마법사 전용 매커닉이라 전부 0코스트였음)
        ["quick_slash"]    = new Card("quick_slash",    "빠른 베기",     1, CardType.Attack,  dmg: 3),
        ["solid_strike"]   = new Card("solid_strike",   "묵직한 일격",   2, CardType.Attack,  dmg: 11),
        ["puncture"]       = new Card("puncture",       "관통",         1, CardType.Attack,  dmg: 6, poison: 1),
        ["guard_up"]       = new Card("guard_up",       "수비 태세",     1, CardType.Defense, blk: 6),
        ["steady_guard"]   = new Card("steady_guard",   "굳건한 수비",   2, CardType.Defense, blk: 11),
        ["counter_stance"] = new Card("counter_stance", "반격 태세",     1, CardType.Defense, blk: 5, draw: 1),
        ["focus"]          = new Card("focus",          "집중",         1, CardType.Skill,   draw: 2),
        ["adrenaline"]     = new Card("adrenaline",     "아드레날린",    1, CardType.Skill,   str: 1, draw: 1),
        ["second_wind"]    = new Card("second_wind",    "재정비",       1, CardType.Skill,   heal: 5),
        ["toughen"]        = new Card("toughen",        "담금질",       1, CardType.Skill,   str: 2),
    };
