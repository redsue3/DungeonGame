...
## 3단계 — 맵 탐색 중 카드 사용 + 기습 매커닉 + 부서지는 벽 (2026-07-10)

**배경**: 위 3단계 항목 착수. 사용자 확인 결과 단순히 "맵에서 카드 쓰기"가 아니라 세 가지가 묶인 요구사항이었음:
1. 전투용 `currentMana`(BattleManager)와 완전히 별개인 새 자원으로 맵 카드 코스트를 관리.
2. 회복 카드처럼 "치명적으로 쉬워지는" 효과만 제외하고 나머지 비공격 카드는 맵에서 쓸 수 있게 하되, 방어막/다음 카드 예약 버프처럼 전투 중에만 의미 있는 효과는 즉시 적용하지 않고 **직전에 쓴 카드 1장**만 기억해뒀다가 다음 조우 시작 시 "기습"으로 1회 발동.
3. 숨겨진(부서지는) 벽 — 공격 카드로 타격하면 부서지고, 일반 벽과 시각적으로 구분됨.

**탐사 코스트 (`System/ExplorationCostSystem.cs`, 신규)**
- `PlayerCharacter.explorationCost`/`maxExplorationCost` 추가 — 캐릭터 생성 시 `maxMana`와 같은 값으로 초기화되지만 이후로는 전투 코스트와 완전히 독립적으로 증감함.
- 회복 규칙은 `HungerSystem`과 동일한 "걸음 수 누적" 패턴을 그대로 재사용: `TilesPerCostTick`(20칸)마다 1 회복. 정확한 회복 속도에 대한 요구사항은 없어서 판단해서 정한 값 — 너무 빠르면 코스트 제한이 무의미해지고 너무 느리면 아예 안 쓰게 될 것 같아 배고픔 틱(60칸)보다 빠르게 잡음. 체감이 안 맞으면 이 상수만 조정하면 됨.
- `SaveSystem`에 `explorationCost`/`maxExplorationCost` 필드 추가, 구버전 세이브 호환은 `maxHunger`와 동일한 패턴(0이면 `PlayerFactory.Create`가 채워둔 기본값을 그대로 둠)으로 처리.

**맵 카드 사용 + 기습 (`DungeonManager.UseCardOnMap`, `BattleManager.ApplyAmbush`)**
- 맵에서 쓸 수 있는 카드 = 공격 카드가 아니고, 회복(`healAmount`)도 자해(`selfDamage`)도 없는 카드 (`DungeonManager.IsMapUsable`). 자해 카드(`dark_pact` 등)는 맵에서 얻는 이득이 없이 페널티만 남아서 회복과 함께 제외.
- 힘(`strengthGain`)은 이 게임에서 원래 전투 사이에도 리셋 안 되는 영구 스탯(`PlayerCharacter.strengthStack`, 세이브도 됨)이라 맵에서 써도 즉시 그대로 적용 — 굳이 "기습"으로 미룰 이유가 없음.
- 방어막(`block`)과 성소 유틸 카드의 다음 카드 예약 버프(`buffNextAttack`/`buffNextDefense`)는 전투 중에만 의미가 있어서, 맵에서 카드를 쓸 때마다 `PlayerCharacter.pendingAmbushCard`에 그 카드 자체를 저장(덮어쓰기 — 항상 마지막 1장만 유지). `BattleManager.StartBattle`이 시작할 때 `ConsumeAmbush()`로 가져가서 보관해뒀다가, 첫 플레이어 턴의 `OnTurnStart()`(방어막 리셋) **직후**에 적용해야 함 — `StartBattle` 시점에 바로 적용하면 첫 턴 `ResetBlock()`에 곧바로 날아가버려서 반드시 이 순서를 지켜야 함.
- 드로우 계열 효과(`drawCount`)는 맵에 핸드 개념이 없어서 그냥 무시됨(카드 자체는 쓸 수 있지만 아무 일도 안 일어남) — 별도 처리 없이 자연스럽게 no-op.
- 카드 사용은 덱에서 카드를 제거하지 않음(드로우/버림 같은 파일 이동이 전혀 없음) — `Deck`의 draw/hand/discard 세 파일 중 어디에 있든 `GetAllCards()`로 조회만 하고 참조만 저장하기 때문에, "덱 순환은 전투 중에만"이라는 요구사항이 별도 분기 없이 자동으로 지켜짐.

**부서지는 벽 (`Map/DungeonFloor.cs`, `Map/FloorGenerator.cs`)**
- `TileKind`에 `BreakableWall` 추가. `FloorGenerator.PlaceHiddenAlcoves`가 방/복도를 다 깎은 뒤, 바닥과 정확히 한 면만 맞닿고 반대편이 사방 전부 벽으로 고립된 벽 한 칸을 찾아서 그 벽을 `BreakableWall`로, 반대편 고립 칸을 `Floor`(숨겨진 알코브)로 바꾸는 방식으로 층당 2개까지 배치. 알코브는 기존 방/복도와 이 벽 한 칸으로만 이어져 있어서 기존 도달가능성(BFS) 불변식엔 영향 없음.
- `DungeonManager.TryBreakWall(dx,dy)` — 맵 UI에서 인접한 부서지는 벽 타일을 클릭하면 호출. 카드 선택 UI 없이, 감당 가능한 것 중 가장 싼 공격 카드를 덱에서 자동으로 골라 소모(탐사 코스트)하고 벽을 부순 뒤 골드 30~60 보상.
- `DungeonMapUI` — 예전엔 `TileKind.Floor`만 실체 오브젝트로 만들고 벽은 아예 안 그렸는데, `BreakableWall`도 같이 실체로 만들되 일반 벽과 다른 갈색 "균열" 타일로 표시해서 구분. 인접했을 때 클릭하면 이동 대신 `TryBreakWall` 호출.

**카드 탭 UI (`System/UI/CardTabUI.cs`, 신규)**
- 인벤토리 버튼 옆에 "카드" 버튼 추가 → 오버레이 패널에서 탐사 코스트 잔량, 현재 기습 예약된 카드, 맵에서 쓸 수 있는 카드 목록(코스트 부족하면 버튼 비활성화)을 보여줌. 카드 프리팹(`CardUI`)을 배틀/보상 화면과 그대로 재사용.
- `Editor/SceneSetup.cs`에 카드 탭 서브패널/버튼 배선 추가, 상단바에 탐사 코스트 텍스트도 추가.

**검증**: 임시 스모크 테스트(확인 후 삭제)로 세 가지 확인 — (1) 배치모드로 1~3계층 각 200회씩 총 600회 맵 생성, 보스방 BFS 도달가능성 유지 + 부서지는 벽이 층당 2개 이하로만 배치되는지, (2) 맵에서 카드 사용 시 코스트 소모/힘 즉시 적용/기습 예약이 올바른지 + 실제 `BattleManager.StartBattle` 호출 시 기습 방어막이 정확히 적용되고 예약이 소모되는지, (3) `TryBreakWall`이 부서지는 벽에서만 성공하고 일반 벽에서는 실패하는지, 골드/코스트 증감이 맞는지. 전부 통과. `SceneSetup.BuildScene`도 배치모드로 재실행해서 새 UI 필드 바인딩 전부 정상 연결되고 씬이 저장되는지 확인함.

**주의 — 테스트 중 로컬 세이브 파일 소실**: 위 (2)/(3) 테스트가 실제 `DungeonManager`/`SaveSystem`을 거치면서 배치모드 중 `%AppData%\LocalLow\DefaultCompany\DungeonGame\save.json`에 테스트용 가짜 진행 상황을 실제로 덮어씀. 테스트 전에 이미 저장돼 있던 진행 상황이 있었다면 복구 불가능하게 사라졌음 — 확인 후 이 파일은 삭제해뒀으니 다음 플레이는 새 런으로 시작하면 됨. 다음에 이런 자동 테스트를 짤 땐 `SaveSystem`을 호출하는 경로(전투 승리, 벽 파괴, 카드 보상 등)를 실제 세이브 파일 대신 임시 경로로 우회시키는 방법을 고민할 것.

**다음에 이어서 할 것 (기존 4단계, 변동 없음)**
- 4단계(추후): 전투 자체를 그리드 위로 통합해서 전투 중 이동/도주/키이팅까지 가능하게.
- (사용자 피드백 대기 중) 기습 매커닉 체감 밸런스 — 방어막만 기습되고 공격 관련 카드는 애초에 맵에서 못 쓰게 막아뒀는데(공격 카드는 벽 파괴 전용), 혹시 "공격 카드도 기습으로 예약해서 전투 시작 즉시 선제공격" 같은 걸 원했다면 범위를 넓혀야 함 — 현재는 방어막/버프 예약 카드만 기습 대상.
